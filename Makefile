...
ifneq ($(CXX),clang-cl)
	...
	# /O2: Reduces binary size a bit when compared to /Od (the default)
	# /MT: Statically link the C++ runtime to avoid runtime dependencies
	# /EHsc: Enable exception handling for TXN_CATCH() (this isn't the default on clang-cl)
	# -m32: Use the 32-bit target
	SP_CFLAGS := /std:c++17 /O2 /MT /EHsc -m32
endif

ifneq ($(CXX),clang-cl)
	# Test stack alignment flags (GCC and Clang have different ones)
	SP_CFLAGS_TEST := $(shell echo 'int main(){}' | $(CXX) -E -mstack-alignment=2 - > /dev/null 2>&1)

	ifeq ($(.SHELLSTATUS),0)
		SP_CFLAGS += -mstack-alignment=2
	endif

	SP_CFLAGS_TEST := $(shell echo 'int main(){}' | $(CXX) -E -mincoming-stack-boundary=2 - > /dev/null 2>&1)
...
ifneq ($(CXX),clang-cl)
	...
	# /LARGEADDRESSAWARE: Enables higher address space for 32-bit
	SP_LDFLAGS := /link /LARGEADDRESSAWARE
endif
